...
for /f "usebackq delims=" %%P in (`"%PYEXE%" -c "import os; from app.bootstrap import find_available_port; print(find_available_port(int(os.environ.get('APP_PORT','3010'))))"`) do set "APP_PORT=%%P"

set "APP_URL=http://127.0.0.1:%APP_PORT%/"
set "LAN_URL=http://%LAN_IP%:%APP_PORT%/"

echo Starting OCT Studio...
if not "%APP_PORT%"=="%REQUESTED_PORT%" echo Port %REQUESTED_PORT% is unavailable, using %APP_PORT% instead.
echo Visit: %APP_URL%
echo LAN: %LAN_URL%
