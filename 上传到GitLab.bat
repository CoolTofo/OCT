...
echo [1/5] 正在连接 GitLab 获取远端状态...
git fetch --prune "%REMOTE%" "%BRANCH%"
if errorlevel 1 (
    echo.
    echo [ERROR] 无法连接 GitLab 或获取远端状态失败，请检查网络、账号登录或 GitLab 访问权限。
    goto :fail
)
...
echo.
echo [4/5] 正在同步 GitLab 最新提交...
git pull --rebase "%REMOTE%" "%BRANCH%"
if errorlevel 1 goto :rebase_failed

echo.
echo [5/5] 正在上传到 GitLab...
...
:rebase_failed
echo.
echo [ERROR] 同步 GitLab 最新提交时发生冲突，已尝试停止 rebase。
